...
class GameAddModal(discord.ui.Modal, title='➕ 新增遊戲'):
    ...
    async def on_submit(self, interaction: discord.Interaction):
        ...
        # 檢查是否已經存在
        if await self.cog.bot.db.has_server_game(self.guild_id, name):
        ...
        count = await self.cog.bot.db.get_server_games_count(self.guild_id)
        if count >= 25:
        ...
        # 存入資料庫
        await self.cog.bot.db.add_server_game(self.guild_id, name)
...
class GameRemoveView(discord.ui.View):
    ...
    async def select_callback(self, interaction: discord.Interaction):
        ...
        await self.cog.bot.db.remove_server_game(self.guild_id, game_to_remove)
...
class GamePanelView(discord.ui.View):
    ...
    async def get_panel_embed(self):
        games = await self.cog.bot.db.get_server_games(self.guild_id)
